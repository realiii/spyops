...
from spyops.query.analysis.extract import QuerySplitByAttributesTable
...
def test_field_names_and_count(request, name, fix_name, count, inserts, selects):
    ...
    element = geo[name]
    fields = [Field('a', data_type=FieldType.text)]
    result = QuerySplitByAttributesTable(element, fields)._field_names_and_count(element)
