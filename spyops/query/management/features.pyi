...
from fudgeo import (
    FeatureClass, Field, MemoryGeoPackage, SpatialReferenceSystem, Table)
...
class QueryCalculateGeometryAttributes(AbstractSourceUpdateQuery):
    ...
    def __init__(self, source: FeatureClass, field: Field,
                 ...
                 area_unit: AreaUnit, where_clause: str) -> None: ...
    def _get_field_names(self) -> NAMES: ...
    @property
...
class AbstractQueryMinimumBoundingGeometry(AbstractQueryGroup, metaclass=ABCMeta):
    ...
    def _process_geometries(self, ids: ndarray, geoms: Union[ndarray, list]) -> dict[int, tuple]: ...
...
class BaseQueryFeatureTo(AbstractSourceQuery):
    ...
    def __init__(self, source: FEATURE_CLASSES, target: FeatureClass, xy_tolerance: XY_TOL) -> None: ...
    @cached_property
    ...
    @property
    def _has_zm(self) -> HasZM: ...
    def _get_lines(self, scratch: MemoryGeoPackage) -> tuple[list, list[QueryFeatureToPrepare], GRID_SIZE]: ...
    @staticmethod
    def _fetch_lines_sizes(query: QueryFeatureToPrepare, lines: list[LineString], sizes: list[GRID_SIZE]) -> None: ...
...
class QueryFeatureToPolygon(BaseQueryFeatureTo):
    ...
    @staticmethod
    def _index_overlay(points: list[ShapelyPoint], planarized: list[Polygon]) -> defaultdict[int, list[int]]: ...
    @staticmethod
...
class AbstractQueryPointsToLine(AbstractQueryGroup):
    ...
    def __init__(self, source: FeatureClass, target: FeatureClass, group_fields: FIELDS, sort_fields: SORT_FIELDS, close_line: bool, is_continuous: bool) -> None: ...
    def _get_target_shape_type(self) -> str: ...
    def _field_names_and_count(self, element: FeatureClass) -> tuple[int, str, str]: ...
    def _build_insert_names(self, element: FeatureClass, fields: FIELDS) -> tuple[int, str]: ...
    def _build_select_names(self, element: FeatureClass, fields: FIELDS) -> tuple[FIELDS, str]: ...
    @property
    def _line_class(self) -> LINE_TYPE: ...
    def _get_coordinates(self, points: defaultdict[int, list[ShapelyPoint]]) -> dict[int, 'ndarray'] | dict[int, list]: ...
    def _build_segments(self, points: defaultdict[int, list[ShapelyPoint]], attributes: defaultdict[int, list[tuple]]) -> list[tuple[LineString, tuple]]: ...
    def _build_lines(self, points: defaultdict[int, list[ShapelyPoint]], attributes: defaultdict[int, list[tuple]]) -> list[tuple[LineString, tuple]]: ...
    def _close_segment_attrs(self, attributes: defaultdict[int, list[tuple]]) -> None: ...
    @abstractmethod
    def _get_segment_attrs(self, attributes: defaultdict[int, list[tuple]]) -> dict[int, list[tuple]]: ...
    @abstractmethod
    def _get_line_attrs(self, attributes: defaultdict[int, list[tuple]]) -> dict[int, tuple]: ...
    @property
    def select_geometry(self) -> str: ...
...
class QueryAdjust3DZ(AbstractSourceUpdateQuery):
    ...
    _adjuster: Callable[['ndarray'], 'ndarray']

    def __init__(self, source: FeatureClass, adjuster: Callable[['ndarray'], 'ndarray'], where_clause: str) -> None: ...
    @property
